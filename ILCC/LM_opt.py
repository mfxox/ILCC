...
def back_project(r_t, img, corners_in_img_arr, corners_in_pcd_arr):
    ...
    s1 = np.zeros(corners_in_img_arr.shape[0]).astype(np.int32) + 10
    s1[0] = 20
    s1[-1] = 20
    ...
    s2 = np.zeros(corners_in_pcd_arr.shape[0]).astype(np.int32) + 5
    s2[0] = 15
    s2[-1] = 15

    cv2.polylines(img, [corners_in_img_arr], 0, (255, 255, 0), 3, lineType=16)
    for i in xrange(corners_in_img_arr.shape[0]):
        cv2.circle(img, tuple(corners_in_img_arr[i].tolist()), s1[i], tuple(c1[i].tolist()), 3)
        cv2.putText(img, str(i + 1), tuple(corners_in_img_arr[i].tolist()), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (255, 255, 255), 2)
    ...
    for i in xrange(proj_corners.shape[0]):
        ...
        cv2.putText(img, str(i + 1), (proj_corners[i][0], proj_corners[i][1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0),
                    1)
    return img
...
def run_min(args, initial_guess=[0, 0, 0, 0, 0, 0]):  # (np.random.random(6)).tolist()
    print
    res = least_squares(cost_func, initial_guess, args=args, method="lm", ftol=1e-10, max_nfev=100000)
